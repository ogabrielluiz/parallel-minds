...
This file is the shared contract every inbox-loops agent reads — loops, dispatcher, and consumers all follow it. Background, philosophy, and the "why" live in the inbox-loops SKILL.md and the plugin's `CONTEXT.md` glossary; this file only spells out the storage model, IDs, and rules. (No relative link: this file is copied into `<inbox-dir>/loops/`, where a repo-relative path would not resolve.) Per-user settings (inbox-dir path, credentials, cadence overrides, archive window) live in `<inbox-dir>/config.yaml`.
